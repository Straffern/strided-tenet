...
#     pdb.set_trace()
    N, d = ngbrs.shape
    # Mask for valid neighbours amongst the d-neighbours
    validNgbrs = (ngbrs >= 0)
    # Row indices like in sparse matrix formats
    row = np.repeat(np.arange(N), d)
    ...
    adj = sp.csr_matrix((np.ones(col.size, dtype=bool), (row, col)), shape=(
        N, N)).toarray()  # Make adj matrix
...
#     pdb.set_trace()
    ...
    # Mask for valid neighbours amongst the d-neighbours
    ngbrs = random*valIdx + ngbrs*(1-valIdx)
    # Row indices like in sparse matrix formats
    row = np.repeat(row, d).reshape(-1)
...
def dice(labels, preds, test=False):
    ...
    acc = 2. * (torch.sum(preds_bin * labels, dim=1) + EPS) / \
        (preds_bin.sum(1) + labels.sum(1)+EPS)
    if test:
        return acc.mean(), acc
    else:
        return acc.mean()
...
def unfold(img: torch.Tensor, shape, has_channels=True):
    """Returns img, with non-overlapping strides specified by shape argument.
        Can omit featureDim/channelDim using has_channels=False.
    Args:
        img (torch.Tensor): image tensor with batch, featureDim, H, W, ...
        shape ([int]): [description]

    Raises:
        Exception: General error that warns about wrong dimensions

    Returns:
        torch.Tensor: 
    """
    dims = len(img.shape)
    offset = 2 if has_channels else 1
    if len(shape)+offset != dims:
        if has_channels:
            raise Exception("unfold expects img to have batch and channel dim.")
        raise Exception("unfold expects img to have batch dim.")
    pieces: torch.Tensor = img
    for i in range(len(shape)):
        pieces = pieces.unfold(i+offset, shape[i], shape[i])

    # return pieces.reshape(-1, *shape)
    return pieces


def fold(folds: torch.Tensor, has_channel=False):
    """Takes an img, that was partitioned into non-overlapping patches/strides, and merges it back to it's original shape/state.

    Args:
        folds (torch.Tensor): image tensor with batch, featuredim and stride/strideWidth pairs. eg: [BatchSize, FeatureSize, 7,7,32,32] for a 224x224 img.
        has_channel (bool, optional): [description]. Defaults to False.

    Returns:
        torch.Tensor: Normal image tensor
    """
    offset = 2 if has_channel else 1
    dims = len(folds.shape)-offset

    output: torch.Tensor = folds
    left = len(folds.shape)//2
    for i in range(dims//2):
        right = len(output.shape)-2
        output = output.movedim(left-i, right-i)
        if i == 0:
            output = output.reshape(*output.shape[:-2-i], -1)
            continue
        output = output.reshape(*output.shape[:-2-i], -1, *output.shape[-i:])
    return output
